...
Deno.serve(async (req) => {
  ...
  let body: { classified_id?: unknown; email?: unknown };
  try {
    body = await req.json();
  } catch {
  ...
  const eventId = String(body.classified_id ?? "").trim();
  const email = String(body.email ?? "").trim().toLowerCase();

  if (!eventId) return jsonResp({ error: "classified_id required" }, 400);
  ...
  const { data: listing, error: listingErr } = await db
    .from("classifieds")
    .select("id, title, contact_email, source")
    .eq("id", eventId)
    .single();
  ...
  if (listing.source !== "user_submitted") {
    return jsonResp({ error: "This listing cannot be edited" }, 403);
  }
  if ((listing.contact_email || "").trim().toLowerCase() !== email) {
  ...
  await db
    .from("classified_otps")
    .delete()
    .eq("classified_id", eventId)
    .eq("used", false);
  ...
  const { error: insertErr } = await db
    .from("classified_otps")
    .insert({ classified_id: eventId, email, code, expires_at: expiresAt });
  ...
  const html = `
    <div style="font-family: Georgia, serif; max-width: 480px; margin: 0 auto; padding: 32px 24px;">
      ...
      <p style="color: #666; font-size: 14px; margin: 0 0 24px;">
        You requested to edit <strong>${listingTitle}</strong> on The Videshi.
      </p>
  ...
  const text = `Your edit code for "${listing.title}" on The Videshi is: ${code}\n\nThis code expires in 10 minutes.\n\nIf you didn't request this, ignore this email.`;
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: `Your Classified Edit Code — The Videshi`,
        html,
        text,
      }),
    });
