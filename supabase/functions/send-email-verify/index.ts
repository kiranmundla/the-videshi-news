...
Deno.serve(async (req) => {
  ...
  /* Delete any existing unused email verifications for this email */
  await db
    .from("event_otps")
    .delete()
    .is("event_id", null)
  ...
  const { error: insertErr } = await db
    .from("event_otps")
    .insert({ event_id: null, email, code, expires_at: expiresAt });

  if (insertErr) {
    console.error("Email verify insert error:", insertErr);
  ...
  const html = `
    <div style="font-family: Georgia, serif; max-width: 480px; margin: 0 auto; padding: 32px 24px;">
      ...
      <p style="color: #666; font-size: 14px; margin: 0 0 24px;">
        Enter this code on The Videshi to verify your email and submit an event.
      </p>
  ...
  const text = `Your verification code for The Videshi Events is: ${code}\n\nThis code expires in 10 minutes.\n\nIf you didn't request this, ignore this email.`;
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: "Verify Your Email \u2014 The Videshi Events",
        html,
        text,
      }),
    });
    ...
    console.error("send-email-verify exception:", e);
