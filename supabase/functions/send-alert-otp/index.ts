...
Deno.serve(async (req) => {
  ...
  const html = `
    <div style="font-family: Georgia, serif; max-width: 480px; margin: 0 auto; padding: 32px 24px;">
      <h2 style="font-size: 20px; color: #1a1a1a; margin: 0 0 8px;">Confirm Your Visa Alerts</h2>
      <p style="color: #666; font-size: 14px; margin: 0 0 24px;">
        Enter this code to subscribe to visa slot alerts on The Videshi.
      </p>
  ...
  const text = `Your verification code for The Videshi visa alerts is: ${code}\n\nThis code expires in 10 minutes.\n\nIf you didn't request this, ignore this email.`;
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: "Confirm your visa alerts — The Videshi",
        html,
        text,
      }),
    });
    ...
    console.error("send-alert-otp exception:", e);
