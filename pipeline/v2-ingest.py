...
CATEGORY_KEYWORDS = {
    "sports": ["cricket","ipl","sports","tennis","match","wicket","goal ","football","soccer","fifa","world cup","athlete","olympics"],
    ...
    "food": ["recipe","biryani","masala","curry","paneer","dosa","tikka","tandoori","indian food","indian restaurant","spice","cooking"],
    "travel": ["airline","flights","airport","tourism","travel advisory","air india","indigo","passport","lounge","hotel"],
    "lifestyle-health": ["health","yoga","wellness","ayurveda","meditation","fitness","mental health","skincare","fashion","beauty"],
}
...
def main():
    ...
    # ── Step 6: Cluster unprocessed signals into topics ────────────────────────
    print(f"\n── Step 6: Clustering unprocessed signals into topics ──")
    unprocessed = sb_get("p2_signals", {
        "is_processed": "eq.false",
        "select": "id,title,feed_source_id,original_url,published_at",
        "order": "fetched_at.desc",
        "limit": "500",
    }, range_header="0-499")

    if not unprocessed or isinstance(unprocessed, dict):
        unprocessed = []

    if not unprocessed:
        print("  No unprocessed signals to cluster.")
    elif DRY_RUN:
        print(f"  [DRY RUN] Would cluster {len(unprocessed)} signals")
    else:
        print(f"  Found {len(unprocessed)} unprocessed signals")

        # Build clusters by keyword overlap
        clusters = {}
        for sig in unprocessed:
            title = sig.get("title", "")
            matching_key = find_matching_cluster(title, clusters)
            if matching_key:
        ...
        for key, sigs in clusters.items():
            ...
            vertical = CATEGORY_VERTICAL_MAP.get(cat, "politics")

            # Score: multi-signal clusters score higher
            sc_sig = min(50 + n * 10, 90)
            sc_src = min(n * 20, 90)
            sc_total = round(70 * 0.25 + sc_sig * 0.3 + 50 * 0.25 + sc_src * 0.2)

            # Extract proper noun keywords for the topic
            ...
            result = sb_post_returning("p2_topics", topic)
            if result and isinstance(result, dict) and result.get("id"):
                topics_created += 1
                tid = result["id"]
                # Link signals to topic
                for sig in sigs:
                    sb_post("p2_topic_signals", {
                        "topic_id": tid,
                        "signal_id": sig["id"],
                    })

            # Mark signals as processed regardless
            for sig in sigs:
                ids_to_mark.append(sig["id"])

        # Batch mark signals as processed
        for i in range(0, len(ids_to_mark), 50):
            batch_ids = ",".join(ids_to_mark[i:i+50])
            sb_patch("p2_signals", {"id": f"in.({batch_ids})"}, {"is_processed": True})
