...
def generate_themed_image(image_prompt, article_id, idx, out_dir="/tmp/videshi_gen",
    ...
    """Generate an article-specific scene image using the OpenAI Responses API
    with integrated image_generation tool. Falls back to legacy gpt-image-1 if
    the Responses API fails.

    When previous_response_id is provided, the model retains full article context
    from the planning phase, producing contextually richer images.
    ...
    try:
        ...
        # ── PRIMARY: Responses API with image_generation tool ──
        resp_url = "https://api.openai.com/v1/responses"
        resp_headers = {"Authorization": f"Bearer {OPENAI_KEY}",
                        "Content-Type": "application/json"}
        resp_body = {
            "model": "gpt-4o",
            "input": f"Generate this scene image: {image_prompt}. No human faces.",
            "tools": [{"type": "image_generation", "size": "1024x1536",
                        "quality": "high"}],
            "tool_choice": {"type": "image_generation"},
        }
        if previous_response_id:
            resp_body["previous_response_id"] = previous_response_id

        r = requests.post(resp_url, headers=resp_headers, json=resp_body,
                          timeout=300)
                ...
                if out.get("type") == "image_generation_call":
                    ...
                    if b64:
                        ...
                        local_png = os.path.join(out_dir,
                                                 f"gen_{article_id}_{idx}.png")
                        local_jpg = os.path.join(out_dir,
                                                 f"gen_{article_id}_{idx}.jpg")
                        with open(local_png, "wb") as fh:
                            fh.write(png)
                        # Compress for faster Shotstack download
                        ...
                        for f in (local_png, local_jpg):
                            try:
                                ...
                                pass
                        # Return (url, response_id) tuple when chaining is needed
                        _new_id = resp_data.get("id")
                        if previous_response_id is not None:
                            return url, _new_id
                        return url
            print(f"  ⚠️ Responses API returned 200 but no image data")
        else:
            print(f"  ⚠️ Responses API failed ({r.status_code}): {r.text[:160]}")

        # ── FALLBACK: Legacy gpt-image-1 (no brand suffix) ──
        print(f"  ⏩ Falling back to legacy gpt-image-1 for scene {idx}")
...
If no article images are available, use "generate" for all scenes."""
    ...
    for i, scene in enumerate(planned_scenes[:n_scenes]):
        ...
        try:
            ...
            print(f"  ⚠️ Scene {scene_num} request failed: {e}")
            continue

        elapsed = time.time() - t0
        if r.status_code != 200:
            print(f"  ⚠️ Scene {scene_num} failed ({r.status_code}, {elapsed:.0f}s): "
                  f"{r.text[:200]}")
            continue

        resp_data = r.json()
        # Chain context for next scene
        ...
        for out in resp_data.get("output", []):
            if out.get("type") == "image_generation_call":
                b64 = out.get("result", "")
                revised = out.get("revised_prompt", "")
                if b64:
                    import base64 as _b64
