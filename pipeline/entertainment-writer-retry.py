#!/usr/bin/env python3
"""Entertainment writer - quick retry with fixed schema"""
...
# Images already resolved from first run
GULLAK_IMAGE = "https://upload.wikimedia.org/wikipedia/commons/5/5b/Vivek_Gomber%2C_Chaitanya_Tamhane_and_Geetanjali_Kulkarni.jpg"
RANVEER_IMAGE = "https://upload.wikimedia.org/wikipedia/commons/3/32/Ranveer_Singh_in_2023_%281%29.jpg"
RASHMIKA_IMAGE = "https://upload.wikimedia.org/wikipedia/commons/4/4b/Rashmika-Mandanna_at_the_music_launch_of_Pushpa_2%2C_2024.jpg"
# SPB - use original image URL (not thumbnail)
SPB_IMAGE = None  # will resolve below

# Try to get SPB image from Wikimedia Commons search (worked last time after retries)
import urllib.parse
try:
    params = {
        "action": "query",
        "generator": "search",
        "gsrsearch": "SP Balasubrahmanyam singer",
        "gsrnamespace": "6",
        "gsrlimit": "5",
        "prop": "imageinfo",
    ...
    if r.status_code == 200:
        ...
        for pid, page in pages.items():
            ...
            mime = ii.get("mime", "")
            if mime.startswith("image/") and mime != "image/svg+xml" and ii.get("width", 0) >= 300:
                SPB_IMAGE = ii.get("thumburl") or ii.get("url", "")
                print(f"  ✓ SPB image: {SPB_IMAGE[:80]}...")
                break
except Exception as e:
    print(f"  ⚠ SPB image error: {e}")

# Also try Wikipedia directly
if not SPB_IMAGE:
    try:
        r = requests.get("https://en.wikipedia.org/api/rest_v1/page/summary/S._P._Balasubrahmanyam",
                         headers={"User-Agent": "TheVideshi/1.0 (thevideshi.com)"}, timeout=10)
        if r.status_code == 200:
            data = r.json()
            SPB_IMAGE = data.get("thumbnail", {}).get("source") or data.get("originalimage", {}).get("source")
            if SPB_IMAGE:
                print(f"  ✓ SPB Wikipedia image: {SPB_IMAGE[:80]}...")
    except:
        pass

if not SPB_IMAGE:
    SPB_IMAGE = ""
    print("  ⚠ No SPB image found")
...
articles.append({
    ...
    "body": """India's most beloved family drama is back. *Gullak* Season 5 premiered on SonyLIV on June 5, making history as the first Indian web series to return for a fifth season. For diaspora audiences who grew up in small-town India, the Mishra household has become something between a memory and a mirror.
...
All seven episodes of *Gullak* Season 5 are now streaming on SonyLIV. The platform is available internationally, making it accessible to diaspora viewers in the US, UK, Canada, and beyond.""",
    ...
    "image_caption": "Geetanjali Kulkarni, who plays matriarch Shanti Mishra in Gullak",
    "image_attribution": "Wikimedia Commons",
    "status": "published",
    "is_editorial": False,
    "published_at": now,
    "sources": [
        {"url": "https://www.sonyliv.com", "name": "SonyLIV"},
        {"url": "https://www.filmfare.com", "name": "Filmfare"},
        {"url": "https://www.mensxp.com", "name": "MensXP"},
        {"url": "https://www.iwmbuzz.com", "name": "IWMBuzz"}
    ],
    "tags": ["gullak", "sonyliv", "tvf", "web series", "mishra family", "streaming"],
})
...
articles.append({
    ...
    "body": """The biggest Indian film of the year just became the biggest streaming event of the year. *Dhurandhar 2: The Revenge*, Aditya Dhar's ₹1,800-crore spy action thriller starring Ranveer Singh, premiered on JioHotstar on June 4 with a special "Raw & Undekha" edition that includes 30 minutes of unseen footage, behind-the-scenes content, and exclusive cast interviews.
...
Social media reactions have been ecstatic since the premiere went live, with fans declaring their "weekend plans scrapped" and settling in for rewatches. The film's re-watch value, already proven during its theatrical run, appears to be translating to the streaming world.

*Dhurandhar 2: The Revenge* is now streaming on JioHotstar in India, with the Netflix India release scheduled for June 19. For NRIs in the US, UK, and Canada, the film is already available on Netflix internationally.""",
    "category": "entertainment",
    "vertical": "entertainment",
    "image_url": RANVEER_IMAGE,
    "image_caption": "Ranveer Singh, who stars as Jakirat Singh Rangi in Dhurandhar 2: The Revenge",
    "image_attribution": "Wikimedia Commons",
    "status": "published",
    "is_editorial": False,
    "published_at": now,
    "sources": [
        ...
        {"url": "https://www.livemint.com", "name": "Livemint"},
        {"url": "https://www.zoomtventertainment.com", "name": "Zoom TV"},
        {"url": "https://www.filmibeat.com", "name": "Filmibeat"}
    ],
    "tags": ["dhurandhar 2", "ranveer singh", "jiohotstar", "ott", "streaming", "spy universe"],
})
...
articles.append({
    ...
    "body": """The trailer for *Cocktail 2* dropped on June 2, and it answers the question that has been hanging over the project since its announcement: this is not a sequel. It is a spiritual successor — same name, same director, completely different story.
...
Kriti remarked that the rumours appeared to be "coming from just one place" and jokingly glanced at Shahid. He protested: "I didn't say anything, guys. Why would I want it that way?"

A separate moment from the launch went viral: Shahid consoling a female fan who broke down in tears while meeting him, embracing her in a hug after signing her autograph. The clip was widely shared, drawing praise across social media.

## The Diaspora Angle

*Cocktail 2* releases in theatres on June 19. It is positioned as a summer romantic entertainer — the kind of film that the original delivered in 2012 and that Bollywood has struggled to produce consistently since. For NRI audiences, the Italian setting, the modern relationship dynamics, and Pritam's soundtrack make it a strong candidate for the summer watchlist.

Written by Tarun Jain and Luv Ranjan, and produced by Maddock Films and Luv Films, *Cocktail 2* arrives in a market that has been starved of mid-budget romantic films. The original proved the formula works. The sequel has the cast and the music to back it up.""",
    "category": "entertainment",
    "vertical": "entertainment",
    "image_url": RASHMIKA_IMAGE,
    "image_caption": "Rashmika Mandanna, who joins the Cocktail 2 love triangle alongside Shahid Kapoor and Kriti Sanon",
    "image_attribution": "Wikimedia Commons",
    "status": "published",
    "is_editorial": False,
    "published_at": now,
    "sources": [
        ...
        {"url": "https://www.bollywoodbubble.com", "name": "Bollywood Bubble"},
        {"url": "https://thebridgechronicle.com", "name": "The Bridge Chronicle"}
    ],
    "tags": ["cocktail 2", "shahid kapoor", "kriti sanon", "rashmika mandanna", "homi adajania", "maddock films"],
})
...
The tributes on SPB's 80th birthday were notable not for what was said but for what wasn't needed. No one had to explain who he was. No one had to list his achievements. The names alone — Ilaiyaraaja, Kamal Haasan, AR Rahman, Shaan — carry the weight of a musical universe that SPB helped build. The fact that his loss still produces this level of grief six years later is perhaps the most honest measure of what he meant.
...
S. P. Balasubrahmanyam was born on June 4, 1946, in Nellore, Andhra Pradesh. He died on September 25, 2020, at the age of 74. The vacuum Ilaiyaraaja described is six years old now. It has not gotten smaller.""",
    ...
    "sources": [
        ...
        {"url": "https://www.zoomtventertainment.com", "name": "Zoom TV"},
        {"url": "https://ianslive.in", "name": "IANS"},
        {"url": "https://thefreedompress.in", "name": "The Freedom Press"}
    ],
    "tags": ["sp balasubrahmanyam", "spb", "ilaiyaraaja", "kamal haasan", "playback singer", "tributes"],
})
