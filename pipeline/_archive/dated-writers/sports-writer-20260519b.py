#!/usr/bin/env python3
"""Sports writer — insert 2 articles with correct schema."""
...
article1 = {
    ...
    "slug": "big-bash-league-chennai-opener-chepauk-bcci-cricket-australia-20260519",
    "category": "sports",
    "body": article1_body,
    "status": "published",
    "published_at": now,
    ...
    "urgency": "analysis",
    "is_featured": False,
    "diaspora_angle": "Indian diaspora in Australia — over 900,000 people — finally see their adopted country's domestic league intersect with the cricket ecosystem they grew up in. BBL privatisation with IPL money could mean NRI fans in Sydney or Melbourne support teams co-owned by the same groups as their IPL favourites.",
    "sources": ["latestly.com", "livemint.com", "cricbuzz.com", "cricket.com.au", "mykhel.com"],
    "tags": ["Big Bash League", "BBL", "BCCI", "Cricket Australia", "Chepauk", "Chennai", "Perth Scorchers", "IPL", "franchise cricket", "privatisation"],
    "topic_id": "3c064511-8b89-4816-84e4-a16bb56971de",
}

# ── ARTICLE 2: IPL Bigger Window ──
article2_id = str(uuid.uuid4())
article2_body = """Arun Singh Dhumal, the chairman of the Indian Premier League, has made the BCCI's ambitions unmistakably clear: when the ICC's current Future Tours Programme expires in 2027, India will push for a significantly larger IPL window.
...
article2 = {
    ...
    "slug": "ipl-expansion-bigger-window-icc-2027-arun-dhumal-20260519",
    "category": "sports",
    "body": article2_body,
    "status": "published",
    "published_at": now,
    ...
    "urgency": "analysis",
    "is_featured": False,
    "diaspora_angle": "A longer IPL season directly impacts NRI viewing habits across the US, UK, Canada, and Australia — more daily cricket content, better global streaming packages, and a shift from bilateral national identity to transnational franchise loyalty that resonates with diaspora experience.",
    "sources": ["cricbuzz.com", "livemint.com", "crictracker.com", "ianslive.in"],
    "tags": ["IPL", "BCCI", "ICC", "Arun Dhumal", "Future Tours Programme", "bilateral cricket", "franchise cricket", "IPL expansion", "cricket calendar"],
    "topic_id": "b148dc02-308f-498d-a59d-f612565f3e10",
}

print("Inserting article 1 (BBL Chennai)...")
r1 = sb_post("p2_articles", article1)
if isinstance(r1, list) and len(r1) > 0:
    print(f"  ✅ Inserted: {article1_id}")
elif isinstance(r1, dict) and r1.get("code"):
    print(f"  ❌ Error: {r1}")
else:
    print(f"  Result: {json.dumps(r1)[:200]}")

print("Inserting article 2 (IPL Window)...")
r2 = sb_post("p2_articles", article2)
if isinstance(r2, list) and len(r2) > 0:
    print(f"  ✅ Inserted: {article2_id}")
...
print(f"\nArticle IDs:")
print(f"  BBL:  {article1_id}")
print(f"  IPL:  {article2_id}")
