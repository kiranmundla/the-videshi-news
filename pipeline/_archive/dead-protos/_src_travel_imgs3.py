#!/usr/bin/env python3
import os, io, requests, subprocess, tempfile, json
...
def compress(b,max_w=1200,q=82):
...
jobs=[
 ("indigo-international-route-suspensions-airspace-southeast-asia-nri-2026",
   ["IndiGo Airbus A320neo aircraft","IndiGo aircraft airport","IndiGo A321neo","IndiGo airline Airbus"],
   ["airport departure board delay","airplane on runway airline"]),
 ("akasa-air-hanoi-southeast-asia-pivot-gulf-freeze-nri-2026",
   ["Akasa Air Boeing 737 MAX","Akasa Air aircraft","Boeing 737 MAX 8 airline India","Akasa Air 737"],
   ["boeing 737 aircraft airline","airplane sky takeoff"]),
]
results={}
for slug,cq,pq in jobs:
    ...
    if not cands:
        for q in pq:
            for c in pexels(q,5):
                print("  pexels:",c["title"][:55]); cands.append((c["url"],c["title"],"Pexels"))
            if cands: break
    picked=None
    for url,title,attr in cands:
        b=download(url)
        if not b: print("  dl fail:",title[:40]); continue
        try: b=compress(b)
        ...
        if final:
            ...
            print("  PICKED:",final,f"({picked['kb']}KB) <-",title[:55])
            break
    results[slug]=picked
print("\n=== SUMMARY ===")
