...
GAI_ENV = load_env("~/workspace/.env.google-ai") or load_env("~/.env.google-ai")
...
GEMINI_KEY = GAI_ENV.get("GOOGLE_AI_API_KEY", "") or GAI_ENV.get("GEMINI_API_KEY", "")
...
# ═══════════════════════════════════════════════════════════════════════════════
# LLM FALLBACK — Gemini (used when OpenAI quota is depleted / 429 insufficient_quota)
# Mirrors the proven OpenAI→Gemini fallback in review-articles.py. Gemini 2.5
# Flash is a *thinking* model; thinkingBudget:0 is REQUIRED or JSON truncates
# mid-string (see AGENTS.md). curl is used so the egress proxy auth is honored.
# ═══════════════════════════════════════════════════════════════════════════════

def gemini_json(prompt, model="gemini-2.5-flash", max_tokens=2000, temperature=0.8, images=None):
    """Call Gemini and parse a JSON object from the response. `images` is an
    optional list of base64 JPEG strings (for vision QA). Returns a dict or None."""
    if not GEMINI_KEY:
        return None
    parts = [{"text": prompt}]
    ...
    try:
        import tempfile
        # Vision payloads are large → write to file and use -d @file (avoids
        # "Argument list too long"; see AGENTS.md review-articles vision note).
        ...
        if "error" in data:
            print(f"  ⚠️ Gemini error: {str(data['error'].get('message',''))[:120]}")
            return None
        text = data["candidates"][0]["content"]["parts"][0]["text"]
        return json.loads(text)
    except Exception as e:
        print(f"  ⚠️ Gemini call/parse error: {e}")
        return None
...
{{
  ...
  "storyboard": [
    ...
    if r.status_code != 200:
        print(f"❌ Script generation failed: {r.status_code} — trying Gemini fallback")
        result = gemini_json(prompt, max_tokens=4000, temperature=0.8)
        if not result or not result.get("script"):
            print("❌ Gemini fallback also failed for script generation")
            return None
        print("  ✅ Script generated via Gemini fallback")
    else:
...
# Music beds are cached locally + in storage so re-renders of the same reel
# length reuse them instead of re-encoding.
MUSIC_BED_DIR = "/tmp/videshi-music-beds"


def prepare_music_bed(music_url, total_duration, target_mean_db=-26.0,
                      fade=1.2, max_peak_db=-1.0):
    """Pre-render a full-length, looped, leveled music bed for a reel.

    WHY THIS EXISTS: Shotstack's ``timeline.soundtrack`` plays its source file
    exactly ONCE and does NOT loop. Our library tracks are ~30s but anchor reels
    run ~55-60s, so the back half of every reel was silent. On top of that, the
    old fixed ``volume: 0.10`` left the bed ~27 dB under the voiceover — inaudible
    on a phone speaker, which read to viewers as "no music at all".

    This builds a bed that (1) loops to cover the FULL reel duration, (2) is
    level-matched to an audible-but-ducked target loudness so it sits cleanly
    under the voiceover, and (3) has gentle fade in/out baked in. The result is
    uploaded and returned as a public URL to drop straight into ``soundtrack``
    at volume 1.0.

    Returns the bed URL, or None on any failure (caller falls back to the raw
    one-shot soundtrack so a bed-build hiccup never blocks a render).
    """
    try:
        os.makedirs(MUSIC_BED_DIR, exist_ok=True)
        dur = max(1.0, round(float(total_duration), 2))
        src_base = os.path.basename(music_url.split("?")[0]).replace(".mp3", "")
        bed_key = f"bed-{src_base}-{int(round(dur))}s-{int(abs(target_mean_db))}db"
        storage_path = f"music-beds/{bed_key}.mp3"
        public_url = f"{STORAGE_BASE}/{storage_path}"

        # Reuse if already in storage (re-render of same reel length).
        try:
            if requests.head(public_url, timeout=10).status_code == 200:
                print(f"  🎚️ Reusing cached music bed ({dur:.0f}s)")
                return public_url
        except Exception:
            pass

        # 1. Fetch source track (curl honors the egress proxy; urllib does not).
        src_path = os.path.join(MUSIC_BED_DIR, f"src-{src_base}.mp3")
        if not os.path.exists(src_path):
            dl = subprocess.run(
                ["curl", "-sS", "-L", "-o", src_path, "--max-time", "60", music_url],
                capture_output=True, text=True, timeout=80)
            if dl.returncode != 0 or not os.path.exists(src_path) or os.path.getsize(src_path) < 1024:
                print(f"  ⚠️ Music bed: source download failed — falling back to raw soundtrack")
                return None

        # 2. Measure source loudness so we hit the target level precisely.
        det = subprocess.run(
        ...
        info = det.stderr
        def _grab(label):
            m = re.search(rf"{label}:\s*(-?\d+\.?\d*) dB", info)
            return float(m.group(1)) if m else None
        src_mean = _grab("mean_volume")
        src_max = _grab("max_volume")
        if src_mean is None:
            src_mean = -18.0
        if src_max is None:
            src_max = -1.0

        # Gain to reach the target ducked level, clamped to keep peaks under
        # max_peak_db so the bed never clips when mixed with the voice.
        gain_db = target_mean_db - src_mean
        if src_max + gain_db > max_peak_db:
            gain_db = max_peak_db - src_max
        gain_db = round(gain_db, 2)

        # 3. Loop to cover the full reel + bake in fades.
        src_dur_p = subprocess.run(
            ["ffprobe", "-v", "error", "-show_entries", "format=duration",
             "-of", "default=nw=1:nk=1", src_path],
            capture_output=True, text=True, timeout=30)
        try:
            src_dur = float(src_dur_p.stdout.strip())
        except Exception:
            src_dur = 30.0
        loops = max(0, int(dur // max(src_dur, 1.0)) + 1)  # -stream_loop is EXTRA plays
        fade_out_start = max(0.0, dur - fade)
        af = (f"volume={gain_db}dB,"
              f"afade=t=in:st=0:d={fade},"
              f"afade=t=out:st={fade_out_start:.2f}:d={fade}")

        bed_path = os.path.join(MUSIC_BED_DIR, f"{bed_key}.mp3")
        enc = subprocess.run(
            ["ffmpeg", "-y", "-hide_banner", "-nostats",
             "-stream_loop", str(loops), "-i", src_path,
             "-t", f"{dur}", "-af", af,
             "-c:a", "libmp3lame", "-b:a", "160k", bed_path],
            capture_output=True, text=True, timeout=120)
        if enc.returncode != 0 or not os.path.exists(bed_path) or os.path.getsize(bed_path) < 1024:
            print(f"  ⚠️ Music bed: encode failed — falling back to raw soundtrack")
            print(f"     {enc.stderr[-300:]}")
            return None

        # 4. Upload the finished bed.
        url = upload_asset(bed_path, storage_path, "audio/mpeg")
        if url:
            print(f"  🎚️ Music bed ready: {dur:.0f}s loop, {gain_db:+.1f} dB "
                  f"(src mean {src_mean:.1f} → target {target_mean_db:.0f} dB)")
            return url
        print(f"  ⚠️ Music bed: upload failed — falling back to raw soundtrack")
        return None
    except Exception as e:
        print(f"  ⚠️ Music bed build error ({e}) — falling back to raw soundtrack")
        return None
...
def _image_looks_like_diagram(path):
    ...
    try:
        ...
        # Hasler-Süsstrunk colorfulness — robust to sepia/tan tint, unlike a raw
        # distinct-color count. A true color photo scores ~15-50; a B&W or sepia
        # archival scan scores near 0 even when it has many faint tonal values.
        ...
        if distinct_colors <= 12:
            return True
        # (4) Desaturated archival/sepia scan: many tonal values but almost no real
        #     color (the B&W "BROKEN WOOD LADDER - NARA" bug, 2026-06-19). The
        #     ladder literally token-matched "broken ladder" and dodged (3) because
        #     it was tan, not pure gray. Real editorial color photos clear ~15+;
        #     8 sits well below any genuine muted photo, so false-positive risk on
        #     real India imagery stays minimal.
        if colorfulness < 8.0:
            return True
        return False
    except Exception:
...
def _card_image_background(url, W, H):
    ...
    try:
        ...
        for y in range(H):
            t = y / max(H - 1, 1)
            # smooth bowl: ~120 alpha at edges, ~40 mid (eased from 150/70 so the
            # backdrop reads brighter while edges still anchor eyebrow/wordmark)
            edge = abs(t - 0.5) * 2  # 0 mid → 1 edges
            sp[0, y] = int(40 + 80 * (edge ** 1.4))
...
def source_storyboard_images(article, storyboard, count=8):
    ...
    if hero and is_url_downloadable(hero) and hero not in used_in_this_reel:
        # Resolution guard for the HOOK frame. The hook fills 1080x1920 with
        # fit=cover, and QA samples the poster at 1.5s — a sub-1080px hero gets
        # upscaled and reads as "pixelated/blurry at 1.5s" (the #1 recurring QA
        # failure). If the hero's shortest side is under MIN_IMAGE_DIM, drop it
        # from the hook and let scene 1 fall through to entity/Commons/Pexels
        # imagery (or the key-point-card floor), all of which are full-res.
        _hero_dims = check_image_resolution(hero)
        if _hero_dims and min(_hero_dims) < MIN_IMAGE_DIM:
            print(f"  🚫 Hero too small for hook ({_hero_dims[0]}x{_hero_dims[1]} < {MIN_IMAGE_DIM}px) — sourcing a crisp scene-1 visual instead")
            hero = ""
    if hero and is_url_downloadable(hero) and hero not in used_in_this_reel:
...
# DEPRECATED (2026-07-03): _BRAND_GEN_SUFFIX killed as part of the Responses API
# visual overhaul. The old suffix forced every gpt-image-1 prompt into a generic
# navy/gold illustration style, preventing rich composed scenes. The Responses API
# now generates contextual, article-specific visuals without style constraints.
                ...
                for cand in gem_data.get("candidates", []):
                    for part in cand.get("content", {}).get("parts", []):
                        if "inlineData" in part:
                            ...
                            local_jpg = os.path.join(out_dir,
                                                     f"gem_{article_id}_{idx}.jpg")
...
# ═══════════════════════════════════════════════════════════════════════════════
# RESPONSES API — FULL SCENE IMAGE GENERATION (Approach B)
# ═══════════════════════════════════════════════════════════════════════════════
# Two-phase pipeline:
#   Phase 1: GPT-4o plans a storyboard (text only, no image generation)
#   Phase 2: For each scene, generate the image via Responses API with
#            previous_response_id for full article context continuity.
# This is the same architecture ChatGPT uses internally — the model that
...
def generate_responses_api_scenes(article, storyboard, article_id,
    ...
    RESP_HEADERS = {"Authorization": f"Bearer {OPENAI_KEY}",
                    "Content-Type": "application/json"}
    ...
    for s in storyboard:
        ...
        card_subtext = s.get("card_subtext", "")
        scene_descriptions.append(
            f"Scene {s.get('scene', '?')}: Narration: \"{narration}\" "
            f"| Key point: \"{card_text}\" — {card_subtext}"
        )
    scenes_text = "\n".join(scene_descriptions)

    plan_prompt = f"""You are designing visuals for a news Instagram Reel by The Videshi.
...
{{"scenes": [
  ...
  {{"scene": 2, "image_source": "generate", "image_description": "detailed visual description for AI generation (200-350 chars — include subject, mood, setting, color palette, composition style)"}},
...
If no article images are available, use "generate" for all scenes."""
    ...
    try:
        ...
        print(f"  ⚠️ Responses API Phase 1 failed: {e}")
        return None, None

    if r.status_code != 200:
        print(f"  ⚠️ Responses API Phase 1 status {r.status_code}: {r.text[:200]}")
        return None, None

    plan_resp = r.json()
    prev_id = plan_resp.get("id")

    # Extract text
    plan_text = ""
    for out in plan_resp.get("output", []):
    ...
    for i, scene in enumerate(planned_scenes[:n_scenes]):
        ...
        print(f"  🖼️ Scene {scene_num}: generating via Gemini Image...", flush=True)

        gen_prompt = (f"{desc}. "
                      f"Photorealistic, cinematic, vertical 9:16 for mobile reels. "
                      f"No human faces. No logos, branding, slogans, or motivational text.")
                    ...
                    for cand in gem_data.get("candidates", []):
                        for part in cand.get("content", {}).get("parts", []):
                            if "inlineData" in part:
                                ...
                                local_png = os.path.join(out_dir,
                                                         f"gem_{article_id}_{i}.png")
                                local_jpg = os.path.join(out_dir,
                                                         f"gem_{article_id}_{i}.jpg")
                                with open(local_png, "wb") as fh:
                                    fh.write(raw)
                                # Upscale to 1080x1920 with Lanczos sharpening
                                upscaled = os.path.join(out_dir,
                                                        f"gem_{article_id}_{i}_up.png")
                                os.system(
                                ...
                                storage_path = f"reel-gen/{article_id}/gem-scene-{i}.png"
                                ...
                                if url:
                                    ...
                                    media_meta[url] = {
                                        "type": "image", "duration": 0,
                                        "is_generated": True, "is_gemini": True,
                                        "curated": True, "generic_pexels": False,
                                    }
                                    elapsed = time.time() - t0
                                    print(f"  ✅ Scene {scene_num} generated via Gemini ({elapsed:.0f}s)")
                                    gemini_ok = True
                                else:
                                    print(f"  ⚠️ Scene {scene_num} Gemini upload failed")
                                break
                        if gemini_ok:
                            break
                    if not gemini_ok:
                        print(f"  ⚠️ Scene {scene_num}: Gemini returned no image data")
                else:
                    print(f"  ⚠️ Scene {scene_num}: Gemini curl failed (rc={gem_r.returncode})")
            except Exception as e:
                print(f"  ⚠️ Scene {scene_num} Gemini error: {e}")

        # ── FALLBACK: OpenAI Responses API ──
        if not gemini_ok and OPENAI_KEY:
            print(f"  ⏩ Scene {scene_num}: falling back to OpenAI Responses API...")
            try:
                r = requests.post(RESP_URL, headers=RESP_HEADERS, json={
                    "model": "gpt-4o",
                    "input": f"Generate this scene image: {gen_prompt}",
                ...
                print(f"  ⚠️ Scene {scene_num} OpenAI fallback failed: {e}")
                continue

            elapsed = time.time() - t0
            if r.status_code != 200:
                print(f"  ⚠️ Scene {scene_num} OpenAI failed ({r.status_code}, {elapsed:.0f}s): "
                      f"{r.text[:200]}")
                continue

            resp_data = r.json()
            prev_id = resp_data.get("id", prev_id)

            for out in resp_data.get("output", []):
                if out.get("type") == "image_generation_call":
                    ...
                    if b64:
                        ...
                        if url:
                            ...
                            media_meta[url] = {
                                "type": "image", "duration": 0,
                                "is_generated": True, "is_responses_api": True,
                                "curated": True, "generic_pexels": False,
                            }
                            print(f"  ✅ Scene {scene_num} generated via OpenAI fallback ({elapsed:.0f}s)")
                        else:
                            print(f"  ⚠️ Scene {scene_num} OpenAI upload failed")

        if not gemini_ok and not OPENAI_KEY:
            print(f"  ⚠️ Scene {scene_num}: no image API available")
...
def source_reel_media_clean(article, storyboard, count=8):
    ...
    # ── C-bis. HOOK BACKGROUND RECOVERY ──────────────────────────────────────
    # If the hero was rejected (too small / unvetted host / not downloadable) the
    # hook (scene 0) is still empty here. Left alone it falls all the way to a
    # FLAT TEXT-FREE NAVY CARD — the dead opening Kiran flagged (vs. the SpaceX
    ...
    if matched_urls[0] is None:
        ...
        #    bare foreign geo (the documented "ghost-town barn" off-topic failure).
        _hook_subj = (_hook_scene.get("media_subject") or "").strip()
        if not _hook_subj:
            _hv = (_hook_scene.get("visual") or "").strip()
            _hook_subj = _hv if _hv and _hv.lower() not in _FOREIGN_GEO_SUBJECTS else ""
        if _hook_subj and _hook_subj.lower() not in _FOREIGN_GEO_SUBJECTS:
            _hml = _load_media_library_lookup()
            if _hml:
                try:
                    _hasset = _hml.find_media(subject=_hook_subj, exclude_concept=True,
                                              min_quality=55, bump_usage=False)
                except Exception:
                    _hasset = None
                # Stills only for the hook (it renders static; a clip's poster is
                # unreliable), and resolution-gated so QA's 1.5s sample is crisp.
                if _hasset and _hasset.get("media_type") != "video":
                    _hu = _hasset.get("url")
                    if _hu and _hu not in used_in_this_reel and is_url_downloadable(_hu):
                        _hd = check_image_resolution(_hu)
                        if not _hd or min(_hd) >= MIN_IMAGE_DIM:
                            try:
                                _hml.find_media(subject=_hasset.get("subject"),
                                                media_type=_hasset.get("media_type"),
                                                exclude_concept=True, bump_usage=True)
                            except Exception:
                                pass
                            matched_urls[0] = _hu
                            used_in_this_reel.add(_hu)
                            media_meta[_hu] = {"type": "image", "duration": 0,
                                               "curated": True, "generic_pexels": False,
                                               "source_url": _hasset.get("source_url", "")}
                            n_lib += 1
                            print(f"  🖼️ Scene 1 (hook): recovered with media-library photo (\"{_hook_subj}\")")
        # 2) Brand-styled GENERATED illustration of the hook's own scene. Use the
        #    scene's image_prompt if present, else synthesize one from its visual /
        #    card_text + headline. The locked _BRAND_GEN_SUFFIX bans text + real
        #    faces and reserves upper/lower thirds for the hook title overlay.
        if matched_urls[0] is None:
            _hprompt = (_hook_scene.get("image_prompt") or "").strip()
            if not _hprompt:
                _hbits = [
                    (_hook_scene.get("visual") or "").strip(),
                    (article.get("headline") or "").strip(),
                ]
                _hseed = next((b for b in _hbits if b), "")
                if _hseed:
            ...
            if _hprompt:
                ...
                if _hgurl:
                    ...
                    media_meta[_hgurl] = {"type": "image", "duration": 0,
                                          "is_generated": True, "curated": True,
                                          "generic_pexels": False}
                    n_gen += 1
                    print(f"  🖌️ Scene 1 (hook): recovered with generated illustration — \"{_hprompt[:48]}\"")
        if matched_urls[0] is None:
            print("  ⚠️ Scene 1 (hook): no photo/illustration recovered — will use branded card bg")
...
""".strip()
    ...
    if hook_meta.get("type") == "video":
        ...
        # 1.5s and repeatedly flagged "pixelated/blurry at 1.5s" — the #1 recurring
        # failure. `zoomIn` magnifies the hero ~5-10% past native scale by 1.5s, so
        # any hero that isn't comfortably above 1080px gets visibly upscaled right
        # where the reviewer looks. Rendering the hook background STATIC keeps it at
        # the sharpest possible cover scale. The hook HTML overlay still has its own
        # zoom/settle animation, so the hook doesn't read as frozen.
        broll_clips.append({
    ...
    # ── CTA: Branded end card ──
    # PRIMARY: Static designed PNG (pixel-perfect logo + tagline + URL)
    # FALLBACK: HTML overlay (for backward compat if PNG not available)
    endcard_url = get_endcard_url()
    if endcard_url:
        end_card_track = {
            "clips": [
                {
        ...
        # Fallback to the old HTML end card
    ...
    # Shotstack's soundtrack plays its source ONCE with no loop, so a 30s track
    # left the back half of a ~58s reel silent. We pre-render a full-length,
    # looped, leveled, faded bed sized to this reel and attach THAT at full
    # volume (the bed is already ducked to sit under the voice). If the bed
    # build fails for any reason, fall back to the raw track — but at an
    # audible volume and without the misleading 30s-only coverage.
    if music_url:
        bed_url = prepare_music_bed(music_url, total_duration)
        if bed_url:
            timeline["soundtrack"] = {
                "src": bed_url,
                "effect": "fadeOut",  # bed already has fades baked in; tiny safety fade
                "volume": 1.0,
            }
        else:
            timeline["soundtrack"] = {
...
# ── Static end card (designed PNG with logo, tagline, URL) ──
# The designed PNG at pipeline/assets/endcard.png is the primary end card.
# It gets uploaded to Supabase once and cached. The HTML fallback
# (build_end_card_html) is kept for backward compatibility but the static
# image is preferred — it's pixel-perfect and doesn't depend on Shotstack's
# HTML renderer.
_ENDCARD_PNG = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "assets", "endcard.png")
_ENDCARD_URL_CACHE = [None]  # mutable default for lazy upload


def get_endcard_url():
    """Upload the designed end card PNG to Supabase (once) and return its
    public URL. Falls back to None if the file doesn't exist or upload fails."""
    if _ENDCARD_URL_CACHE[0]:
        return _ENDCARD_URL_CACHE[0]
...
{{
  ...
  "notes": "brief summary"
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"  ⚠️ QA API error: {r.status_code} — trying Gemini vision fallback")
            # Reconstruct the text prompt + frames for Gemini vision.
            qa_text = "\n".join(
                part["text"] for part in content if part.get("type") == "text"
            )
            result = gemini_json(
                qa_text, max_tokens=600, temperature=0,
                images=[f["b64"] for f in frames],
            )
            if not result:
                print("  ⚠️ Gemini vision QA fallback also failed — fail closed")
                return False, 0, "QA failed (OpenAI + Gemini both failed — fail closed)"
            print("  ✅ QA gate scored via Gemini vision fallback")
        else:
            result = json.loads(r.json()["choices"][0]["message"]["content"])
        score = result.get("score", 5)
...
def run_anchor_reel(article, dry_run=False, use_production=False, no_publish=False):
    ...
    # ── PRIMARY: Responses API integrated image generation (Approach B) ──
    ...
    if storyboard and USE_RESPONSES_API and OPENAI_KEY:
        ...
        resp_urls, resp_meta = generate_responses_api_scenes(
            article, storyboard, article.get("id", ""))
        if resp_urls and len(resp_urls) >= 3:
            image_urls = resp_urls
            media_meta = resp_meta or {}
            print(f"  ✅ Responses API produced {len(image_urls)} scene images")
        else:
            print(f"  ⚠️ Responses API produced only {len(resp_urls) if resp_urls else 0} "
                  f"images — falling back to CLEAN mode")
            USE_RESPONSES_API = False  # fall through to CLEAN

    REEL_CLEAN = os.environ.get("VIDESHI_REEL_CLEAN", "1") != "0"
    if not USE_RESPONSES_API or not media_meta:
