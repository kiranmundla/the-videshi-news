    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as fd:
            ...
            for fi in range(n):
                ...
                # Dynamic font scaling: shrink if text overflows frame width
                max_tw=W-120  # 60px padding each side
                bw=d.textlength(disp,font=bigf)
                if bw>max_tw:
                    scaled_sz=max(int(220*(max_tw/bw)),72)
                    bigf=_font(scaled_sz,"extrabold")
                    bw=d.textlength(disp,font=bigf)
